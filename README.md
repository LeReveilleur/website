...
Save the list of all donors as a text file with one nickname per line and copy
it under `scripts/data/donors.txt`. Blank lines are ignored and duplicates are
kept as-is. Windows line endings are fine.

The introduction paragraph shown above the list lives in
`scripts/data/donors_intro.md`. Edit that file to change the wording.
...
The content page located at `content/page/donors/index.md` will be overwritten
with the list of donors, sorted alphabetically and grouped into sections by
letter range (A à D, E à H, ...). Names that do not start with a letter end up
in the "Autres" section.

Do not edit `content/page/donors/index.md` by hand: the whole file, including
its frontmatter (title, slug, date, sidebar menu entry), is generated by
`scripts/lereveilleur/donors.clj` and any manual change is lost on the next
run. The page date is set to the day the task runs.

Commit `scripts/data/donors.txt`, `scripts/data/donors_intro.md` and
`content/page/donors/index.md`.
